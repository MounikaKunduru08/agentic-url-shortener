...
public class UrlService {
    ...
    public ShortUrl shorten(String destination) {
        ...
        if (isUnsafeHost(uri.getHost()))
            throw new IllegalArgumentException("destination must not target localhost or a private network");
    ...
    public ShortUrl resolve(String code) {
        ShortUrl url = repository.findByCode(code).orElseThrow(() -> new UrlNotFoundException(code));
        url.recordRedirect();
        return url;
    }

    public ShortUrl analytics(String code) {
        return repository.findByCode(code).orElseThrow(() -> new UrlNotFoundException(code));
    }
