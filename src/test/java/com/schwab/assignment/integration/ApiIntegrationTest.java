...
class ApiIntegrationTest {
  ...
  @Test void workflowReportProvidesLedgerAndDerivedActionCounts() throws Exception {
    MvcResult created=mvc.perform(post("/api/workflows").contentType(MediaType.APPLICATION_JSON)
        .content("{\"scenario\":\"greenfield\",\"requirement\":\"Build URL shortener\"}"))
      .andExpect(status().isOk()).andReturn();
    UUID id=UUID.fromString(mapper.readTree(created.getResponse().getContentAsString()).get("id").asText());

    mvc.perform(get("/api/workflows/{id}/report",id)).andExpect(status().isOk())
      .andExpect(jsonPath("$.workflowId").value(id.toString()))
      .andExpect(jsonPath("$.ledger[*].action",hasItem("CREATED")))
      .andExpect(jsonPath("$.actionCounts.CREATED").value(1));
  }

  @Test void operatorCanPauseResumeAndAbortAWorkflow() throws Exception {
    MvcResult created=mvc.perform(post("/api/workflows").contentType(MediaType.APPLICATION_JSON)
        .content("{\"scenario\":\"greenfield\",\"requirement\":\"Build URL shortener\"}"))
      .andExpect(status().isOk()).andReturn();
    UUID id=UUID.fromString(mapper.readTree(created.getResponse().getContentAsString()).get("id").asText());
    mvc.perform(post("/api/workflows/{id}/pause",id).contentType(MediaType.APPLICATION_JSON).content("{\"operator\":\"control-center\"}"))
      .andExpect(status().isOk()).andExpect(jsonPath("$.status").value("PAUSED"));
    mvc.perform(post("/api/workflows/{id}/resume",id).contentType(MediaType.APPLICATION_JSON).content("{\"operator\":\"control-center\"}"))
      .andExpect(status().isOk()).andExpect(jsonPath("$.status").value("AWAITING_APPROVAL"));
    mvc.perform(post("/api/workflows/{id}/abort",id).contentType(MediaType.APPLICATION_JSON).content("{\"operator\":\"control-center\",\"reason\":\"demo stop\"}"))
