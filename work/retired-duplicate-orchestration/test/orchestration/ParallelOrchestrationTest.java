...
class ParallelOrchestrationTest {
    @Test
    void independentImplementAndTestStagesOverlapBeforeReleaseGate() {
        ...
        StageExecutor executor = (stage, workflow) -> {
            if (stage.id().equals("implement") || stage.id().equals("test")) {
                ...
                independentStagesStarted.countDown();
                try {
                    if (!independentStagesStarted.await(1, TimeUnit.SECONDS))
                        return new StageExecutor.ExecutionResult(false, "stages did not overlap");
                } catch (InterruptedException exception) {
                    Thread.currentThread().interrupt();
                    return new StageExecutor.ExecutionResult(false, "parallelism check interrupted");
                }
            }
